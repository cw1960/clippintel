...
// Helper to map Supabase Profile to UserProfile
function mapProfileToUserProfile(profile: any): UserProfile {
  return {
    id: profile.id,
    email: profile.email,
    ...
    team_role: profile.team_role || undefined,
  };
}
    ...
    (set, get) => ({
      ...
      updateProfile: async (updates: Partial<UserProfile>) => {
        ...
        try {
          ...
          if (data) {
            set({ profile: mapProfileToUserProfile(data), loading: false });
            return { success: true };
          }
      ...
      checkAuthStatus: async () => {
        ...
        try {
          ...
          if (user && profile) {
            ...
            set({
              user,
              profile: mapProfileToUserProfile(profile),
              session,
              loading: false,
              initialized: true,
            });
      ...
      loadUserProfile: async () => {
        ...
        try {
          ...
          if (profile) {
            set({ profile: mapProfileToUserProfile(profile) });
          }
        } catch (error) {
          console.error("Load user profile error:", error);
        }
      ...
      createDefaultProfile: async (user: any) => {
        try {
          ...
          if (data) {
            set({ profile: mapProfileToUserProfile(data) });
          }
        } catch (error) {
          console.error("Create default profile error:", error);
        }
